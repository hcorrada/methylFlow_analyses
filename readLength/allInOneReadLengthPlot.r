...
for (i in 1:ntrees) {
  ...
  points(readLengthAvg_hard$var[sel[seq(1,length(sel),by=2)]],
  ...
  points(readLengthAvg_moderate$var[sel[seq(1,length(sel),by=2)]],
  ...
  points(readLengthAvg_simple$var[sel[seq(1,length(sel),by=2)]],
...
#title = "thresholds",
pch = rep(pchs,3),
col = c("red","red","red", "black", "black", "black", "blue", "blue","blue"),
cex = 0.7,
pt.cex = 0.7,
lty = 3,
lwd= 1.0)
...
for (i in 1:ntrees) {
  ...
  sel <- which(abs(readLengthAvg_hard$threshold - j) < 0.0001)
  points(readLengthAvg_hard$var[sel[seq(1,length(sel),by=2)]],
         readLengthAvg_hard$methylCallError[sel[seq(1,length(sel),by=2)]],
         col = "red",
         pch = pchs[i],
         cex = 0.5,
         type = "b",
  ...
  sel <- which(abs(readLengthAvg_moderate$threshold - j) < 0.0001)
  points(readLengthAvg_moderate$var[sel[seq(1,length(sel),by=2)]],
         readLengthAvg_moderate$methylCallError[sel[seq(1,length(sel),by=2)]],
         col = "black",
         pch = pchs[i],
         cex = 0.5,
         type = "b",
  ...
  sel <- which(abs(readLengthAvg_simple$threshold - j) < 0.0001)
  points(readLengthAvg_simple$var[sel[seq(1,length(sel),by=2)]],
        readLengthAvg_simple$methylCallError[sel[seq(1,length(sel),by=2)]],
        col = "blue",
        pch = pchs[i],
        cex = 0.5,
        type = "b",
