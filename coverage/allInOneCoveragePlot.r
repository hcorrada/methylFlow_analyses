...
for (i in 1:ntrees) {
  ...
  points(coverageAvg_hard$var[sel[seq(1,length(sel),by=2)]],
  ...
  sel <- which(abs(coverageAvg_moderate$threshold - j) < 0.001)
  points(coverageAvg_moderate$var[sel[seq(1,length(sel),by=2)]],
  ...
  points(coverageAvg_simple$var[sel[seq(1,length(sel),by=2)]],
...
plot(0, 0,
     ...
     xlim = c(xrange_min, xrange_max),
     xlab="Coverage",
     ylab="Average Methyl Call Error",
     cex.lab= 1.2
     #     cex.axis = 1.5
)
...
for (i in 1:ntrees) {
  ...
  sel <- which(abs(coverageAvg_hard$threshold - j) < 0.0001)
  points(coverageAvg_hard$var[sel[seq(1,length(sel),by=2)]],
         coverageAvg_hard$methylCallError[sel[seq(1,length(sel),by=2)]],
         col = "red",
         pch = pchs[i],
         cex = 0.5,
         type = "b",
  ...
  points(coverageAvg_moderate$var[sel[seq(1,length(sel),by=2)]],
         coverageAvg_moderate$methylCallError[sel[seq(1,length(sel),by=2)]],
         col = "black",
         pch = pchs[i],
         cex = 0.5,
         type = "b",
  ...
  sel <- which(abs(coverageAvg_simple$threshold - j) < 0.0001)
  points(coverageAvg_simple$var[sel[seq(1,length(sel),by=2)]],
        coverageAvg_simple$methylCallError[sel[seq(1,length(sel),by=2)]],
        col = "blue",
        pch = pchs[i],
        cex = 0.5,
        type = "b",
