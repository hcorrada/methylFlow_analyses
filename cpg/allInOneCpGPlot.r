...
for (i in 1:ntrees) {
  ...
  points(cpgAvg_hard$var[sel[seq(1,length(sel),by=2)]],
  ...
  points(cpgAvg_moderate$var[sel[seq(1,length(sel),by=2)]],
  ...
  points(cpgAvg_simple$var[sel[seq(1,length(sel),by=2)]],
...
for (i in 1:ntrees) {
  ...
  sel <- which(abs(cpgAvg_hard$threshold - j) < 0.0001)
  points(cpgAvg_hard$var[sel[seq(1,length(sel),by=2)]],
         cpgAvg_hard$methylCallError[sel[seq(1,length(sel),by=2)]],
         col = "red",
         pch = pchs[i],
         cex = 0.5,
         type = "b",
  ...
  sel <- which(abs(cpgAvg_moderate$threshold - j) < 0.0001)
  points(cpgAvg_moderate$var[sel[seq(1,length(sel),by=2)]],
         cpgAvg_moderate$methylCallError[sel[seq(1,length(sel),by=2)]],
         col = "black",
         pch = pchs[i],
         cex = 0.5,
         type = "b",
  ...
  sel <- which(abs(cpgAvg_simple$threshold - j) < 0.0001)
  points(cpgAvg_simple$var[sel[seq(1,length(sel),by=2)]],
        cpgAvg_simple$methylCallError[sel[seq(1,length(sel),by=2)]],
        col = "blue",
        pch = pchs[i],
        cex = 0.5,
        type = "b",
